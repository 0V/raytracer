...
namespace simple_mlt
{
    ...
    // *** レンダリングするシーンデータ ****
    // from small ppt    // レンダリングするシーンデータ scene_for_sppm
    Sphere spheres[] = {
        Sphere(5, Vec(50.0, 75.0, 81.6), Color(12, 12, 12), Color(),
               DIFFUSE),                                                                     //照明
    ...
    public:
        ...
        void InitUsedRandCoords() { used_rand_coords = 0; }
    ...
    double luminance(const Color &color) { return Dot(Vec(0.2126, 0.7152, 0.0722), color); }
    ...
    Color direct_radiance_sample(const Vec &v0, const Vec &normal, const int id, KelemenMLT &mlt)
    {
        ...
        const Vec light_pos =
            spheres[LightID].position +
            ((spheres[LightID].radius + EPS) * Vec(sqrt(1.0 - r2 * r2) * cos(r1), sqrt(1.0 - r2 * r2) * sin(r1), r2));
        ...
        if (dot0 >= 0 && dot1 >= 0)
        {
            ...
            {
                return Multiply(spheres[id].color, spheres[LightID].emission) * (1.0 / PI) * G /
                       (1.0 / (4.0 * PI * pow(spheres[LightID].radius, 2.0)));
            }
        }
        return Color();
    }
    ...
    Color radiance(const Ray &ray, const int depth, KelemenMLT &mlt)
    {
        ...
        if (depth > MaxDepth)
        {
            if (mlt.NextSample() >= russian_roulette_probability) return Color();
        }
        else
        ...
        switch (obj.ref_type)
        {
            case DIFFUSE:
            {
                ...
                if (id != LightID)
                {
                    ...
                    {
                        direct_light =
                            direct_light + direct_radiance_sample(hitpoint, orienting_normal, id, mlt) / shadow_ray;
                    }
                    ...
                    return (direct_light + Multiply(obj.color, radiance(Ray(hitpoint, dir), depth + 1, mlt))) /
            ...
            case SPECULAR:
            {
                ...
                Vec direct_light;
                if (lid == LightID) direct_light = spheres[LightID].emission;

                return (direct_light + Multiply(obj.color, radiance(reflection_ray, depth + 1, mlt))) /
                       russian_roulette_probability;
            }
            break;
            case REFRACTION:
            {
                ...
                if (lid == LightID) direct_light_refraction = spheres[LightID].emission;
                ...
                if (depth > 2)
                {
                    ...
                    {  // 屈折
                        return Multiply(obj.color,
                                        (direct_light_refraction + radiance(refraction_ray, depth + 1, mlt)) * Tr) /
                ...
                {  // 屈折と反射の両方を追跡
                    return Multiply(obj.color,
                                    (direct_light + radiance(reflection_ray, depth + 1, mlt)) * Re +
                                        (direct_light_refraction + radiance(refraction_ray, depth + 1, mlt)) * Tr) /
                           russian_roulette_probability;
                }
            }
    ...
    PathSample generate_new_path(const camera_info &cam_info, KelemenMLT &mlt, int x, int y)
    {
        const Ray &camera = cam_info.camera;
        const int width = cam_info.width;
        const int height = cam_info.height;
        const auto &screen_center = cam_info.screen_center;
        const auto &screen_x = cam_info.screen_x;
        const auto &screen_y = cam_info.screen_y;
        const auto &camera_position = cam_info.camera_position;
        ...
        const Vec screen_position =
            screen_center + screen_x * ((r1 + x) / width - 0.5) + screen_y * ((r2 + y) / height - 0.5);
        // レイを飛ばす方向
        const Vec dir = Normalize(screen_position - camera_position);

        //        Vec dir = cx * (((sx + 0.5 + dx) / 2.0 + x) / width - 0.5) + cy * (((sy + 0.5 + dy) / 2.0 + y) /
        //        height - 0.5) +

        //                  camera.dir;
        //        const Ray ray = Ray(camera.org + dir * 130.0, Normalize(dir));
        const Ray ray = Ray(camera_position, dir);
    ...
    // MLTする
    // 画像平面上で大域的に変異させる
    void render_mlt(const int mlt_num, const int mutation, Color *image, const camera_info &cam_info)
    {
        const Ray &camera = cam_info.camera;
        const int width = cam_info.width;
        const int height = cam_info.height;
        ...
        for (int mi = 0; mi < mlt_num; mi++)
        {
            ...
            if (SeedPathMax <= 0) SeedPathMax = 1;
            ...
            for (int i = 0; i < SeedPathMax; i++)
            {
                ...
                PathSample sample = generate_new_path(cam_info, mlt, -1, -1);
                mlt.global_time++;
            ...
            for (int i = 0; i < M; i++)
            {
                if ((i + 1) % (M / 10) == 0)
                {
                    ...
                    std::cout << "Accept: " << accept << " Reject: " << reject
                              << " Rate: " << (100.0 * accept / (accept + reject)) << "%" << std::endl;
                }

                // この辺も全部論文と同じ（Next()）
                mlt.large_step = rand01() < p_large;
                mlt.InitUsedRandCoords();
                PathSample new_path = generate_new_path(cam_info, mlt, -1, -1);
                ...
                tmp_image[new_path.y * width + new_path.x] =
                    tmp_image[new_path.y * width + new_path.x] + new_path.weight * new_path_weight * new_path.F;
                tmp_image[old_path.y * width + old_path.x] =
                    tmp_image[old_path.y * width + old_path.x] + old_path.weight * old_path_weight * old_path.F;

                if (rand01() < a)
                {  // 受理
                    accept++;
                    old_path = new_path;
                    if (mlt.large_step) mlt.large_step_time = mlt.global_time;
                    mlt.global_time++;
                    while (!mlt.primary_samples_stack.empty())  // スタック空にする
                        mlt.primary_samples_stack.pop();
                }
    ...
    inline int to_int(double x) { return int(pow(clamp(x), 1 / 2.2) * 255 + 0.5); }
    ...
    void save_ppm_file(const std::string &filename, const Color *image, const int width, const int height)
    {
        ...
        for (int i = height - 1; i >= 0; i--)
        {
            ...
            {
                auto pid = i * width + j;
                fprintf(f, "%d %d %d ", to_int(image[pid].x), to_int(image[pid].y), to_int(image[pid].z));
            }
        }
        fclose(f);
    }
...
#include <time.h>
    int render()
    {
        int width = 320;
        int height = 240;
        int mutation = 128 * width * height;
        int mlt_num = 10;  // 何回MLTするか
        // カメラ位置
        ...
        // カメラ位置
        Ray camera(camera_position, camera_dir);
        ...
        auto cam_info = camera_info{camera,    width,    height,   camera_position, camera_dir,
                                    camera_up, screen_x, screen_y, screen_center};

        render_mlt(mlt_num, mutation, image, cam_info);
        ...
        sprintf(buf_ppm, "%04d_%d.ppm", mutation, t);
        sprintf(buf, "%04d_%d.hdr", mutation, t);
        save_ppm_file(buf_ppm, image, width, height);
