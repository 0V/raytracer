...
namespace ppm_sample
{
    ...
    public:
        ...
        void CopyToVector(std::vector<T *> &vec) { copy_to_vector(root, vec); }
    ...
    struct Hitpoint
    {
        ...
        Hitpoint(const Vec &position_, const Vec &normal_, const Vec &incident_, const int id_, const int image_index_,
                 const double weight_, const Vec &emission_color_)
            : position(position_),
              normal(normal_),
              incident(incident_),
              id(id_),
    ...
    void trace_scene(const Ray &ray, const int image_index, HitpointMap *hitpoints, const double weight,
                     const int depth)
    {
        double t;  // レイからシーンの交差位置までの距離
        int id;    // 交差したシーン内オブジェクトのID
        if (!intersect_scene(ray, &t, &id)) return;
        ...
        if (depth > MaxDepth)
        {
            if (rand01() >= russian_roulette_probability) return;
        }
        else
        ...
        switch (obj.ref_type)
        {
            case DIFFUSE:
            {
                hitpoints->AddPoint(Hitpoint(hitpoint, normal, ray.dir, id, image_index,
                                             weight / russian_roulette_probability, obj.emission));
                return;
            }
            break;
            ...
            case SPECULAR:
            {
                ...
                trace_scene(Ray(hitpoint, ray.dir - normal * 2.0 * Dot(normal, ray.dir)), image_index, hitpoints,
                            weight / russian_roulette_probability, depth + 1);
                return;
            }
            break;
            case REFRACTION:
            {
                ...
                {  // 全反射した
                    // 反射方向から放射輝度をもらってくる
                    trace_scene(reflection_ray, image_index, hitpoints, weight / russian_roulette_probability,
                                depth + 1);
                    return;
                }
                // 屈折していく方向
                ...
                if (depth > 2)
                {
                    ...
                    {  // 反射
                        trace_scene(reflection_ray, image_index, hitpoints,
                                    Re * weight / probability / russian_roulette_probability, depth + 1);
                        return;
                    }
                    else
                    {  // 屈折
                        trace_scene(Ray(hitpoint, tdir), image_index, hitpoints,
                                    Tr * weight / (1.0 - probability) / russian_roulette_probability, depth + 1);
                        return;
                    }
                }
                else
                {  // 屈折と反射の両方を追跡
                    trace_scene(reflection_ray, image_index, hitpoints, Re * weight / russian_roulette_probability,
                                depth + 1);
                    trace_scene(Ray(hitpoint, tdir), image_index, hitpoints, Tr * weight / russian_roulette_probability,
                                depth + 1);
                    return;
                }
            }
    ...
    void trace_photon(HitpointMap &hitpoints, const int shoot_photon_num, const double Alpha,
                      const double gather_hitpoint_radius, const int gather_max_hitpoints)
    {
            ...
            for (; !trace_end;)
            {
                ...
                switch (obj.ref_type)
                {
                    case DIFFUSE:
                    {
                        ...
                        HitpointMap::Query query(hitpoint, orienting_normal, gather_hitpoint_radius,
                                                 gather_max_hitpoints);
                        ...
                        for (; !hqueue.empty();)
                        {
                            HitpointMap::ElementForQueue p = hqueue.top();
                            hqueue.pop();
                            hits.push_back(p);
                        }
                        ...
                        {
                            if (hits[hi].point->N == 0)
                            {  // まだフォトンが一個も集められていなかった場合だけ入る
                                // 初期値設定
                                hits[hi].point->N = 1;
                                hits[hi].point->R = (hits[hi].point->position - hitpoint).Length();
                                // std::cout << hits[hi].point->R << " ";
                                const Color v = Multiply(spheres[hits[hi].point->id].color, now_flux) /
                            ...
                            {  // 二回目以降に入る
                                ...
                                if ((hits[hi].point->position - hitpoint).Length() < hits[hi].point->R)
                                {
                                    ...
                                    // 新しく追加されたフォトンの寄与計算
                                    const Color tauM = Multiply(spheres[hits[hi].point->id].color, now_flux) /
                                                       PI;  // Diffuse面のBRDF = 1.0 / πであったのでこれをかける
                                    const Color newtau =
                                        (hits[hi].point->accumulated_color + tauM) * ((N + Alpha * M) / (N + M));
    ...
    void update_image(const int iteration_num, Color *image, int width, int height, std::vector<Hitpoint *> &hitpoints)
    {
        for (int hi = 0; hi < hitpoints.size(); hi++)
        {
            ...
            if (hitpoints[hi]->N > 0)
            {
                image[hitpoints[hi]->image_index] = image[hitpoints[hi]->image_index] + hitpoints[hi]->emission_color +
                                                    hitpoints[hi]->weight * hitpoints[hi]->accumulated_color /
                                                        (PI * pow(hitpoints[hi]->R, 2)) / (iteration_num + 1);
            }
        }
    }
    ...
    int render()
    {
        ...
        for (int i = 0; i < iteration; i++)
        {
            ...
            for (int j = 0; j < width * height; j++) image[j] = Color();
